...
\pagestyle{fancy}
\fancyhf{}
\rhead{Laboratoire de systèmes asservis}
...
\begin{titlepage}
  ...
  \rule{\linewidth}{0.5pt}\\[0.4cm]
      {\huge\bfseries Laboratoire 4 de systèmes asservis\\}
  \vspace{1cm}
        {\Large\bfseries Dimensionnement d'un régulateur PI pour moteur CC}
...
Les moteurs électriques à courant continu sont omniprésents dans l'industrie : robots, véhicules
...
\[
  G_{\text{BO}}(s) = G_R(s) \cdot \frac{K_{\text{élé}} \cdot K_{\text{mot}} \cdot K_{\text{mes}}}
      {(1 + s T_{\text{élé}})(1 + s T_1)} = G_R(s) \cdot \frac{1,008}
...
\[
  T_{r,\text{th}} 
  = \frac{3}{\zeta\,\omega_n}
  = \frac{3}{\zeta}\sqrt{\frac{T_n\,T_2}{K_p\,K_s}}
  = \frac{3}{0{,}69}\sqrt{\frac{8\cdot10^{-3}\cdot1\cdot10^{-3}}{3{,}97\cdot1{,}008}}
  \approx 6{,}1\,\text{ms}
\]
...
pour un moteur à courant continu. Après l'obtention de la fonction de transfert du système, nous avons dimensionné un régulateur PI
permettant d'obtenir un dépassement théorique de 5\,\% et un écart statique nul. Une simulation Simulink a permit de validr les performances
($T_r \approx 6$\,ms, $u_{cm,\text{max}} \approx 0{,}46$\,V.
\\\\
Afin de vérifier expérimentalement le modèle théorique, l'expérience sur l'installation réelle a montré un dépassement légèrement supérieur
(7--8\,\%) dû principalement au retard numérique et aux frottements non modélisés. En prenant en compte le retard, 
le redimensionnement a conduit à réduire le gain
    ($K_p : 3{,}97 \to 2{,}65$). Cela a éliminé le dépassement expérimental au prix d'une légèrediminution de la rapidité.
\\\\

Cette expérience illustre bien l'importance de modéliser correctement tous les éléments du système
(y compris les retards de calcul) pour obtenir un comportement en boucle fermée conforme aux attentes.
