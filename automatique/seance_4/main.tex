...
% --------------------------------------------------------------------------
\section{A : Fonction de transfert du système}

\subsection{Gain statique global en boucle ouverte (TO CHECK)}
...
% --------------------------------------------------------------------------
\section{Tâche B -- Dimensionnement du régulateur}
...
\subsection{Méthode de compensation de pôle dominant}

On choisit $T_n = T_1 = 8 \cdot 10^{-3}$\,s afin de compenser le pôle dominant du moteur.
La boucle ouverte compensée devient un système du premier ordre avec intégration :
...
\subsection{Calcul de $K_p$ via le critère de phase}

Un dépassement de 5\,\% correspond à un facteur d'amortissement $\zeta \approx 0{,}69$, soit une marge
de phase $\varphi_m \approx 65^{\circ}$.

En appliquant la méthode de Bode avec le modèle de second ordre équivalent, on obtient la constante
intégrale :
...
où $T_2 = 1 \cdot 10^{-3}$\,s (constante de temps de l'amplificateur, cas sans retard). Numériquement :
...
\subsection{Résumé des paramètres du régulateur PI}

\begin{table}[H]
  \centering
  \caption{Paramètres du régulateur PI (sans retard)}
  \begin{tabular}{lll}
    \toprule
    Paramètre & Valeur & Description \\
    \midrule
    $T_n$ & $8 \cdot 10^{-3}$ s & Temps d'action intégrale \\
    $T_i$ & $\approx 2{,}016 \cdot 10^{-3}$ s & Constante de temps intégrale \\
    $K_p$ & $\approx 3{,}97$ & Gain proportionnel \\
    \bottomrule
  \end{tabular}
\end{table}

% --------------------------------------------------------------------------
\section{Tâche C -- Simulation Simulink (sans retard)}

\subsection{Schéma Simulink}

La boucle de régulation a été modélisée sous Simulink avec le régulateur PI calculé ci-dessus.
La figure~\ref{fig:schema_simulink} présente le schéma bloc utilisé.
...
\subsection{Réponse indicielle simulée}

Un échelon de consigne de 0 à 10\,rad/s est appliqué. La figure~\ref{fig:B_step} montre la réponse
indicielle théorique obtenue par simulation.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{B_step_(G_th).png}
  \caption{Réponse indicielle simulée -- régulateur PI sans retard}
  \label{fig:B_step}
\end{figure}
...
\begin{table}[H]
  \centering
  \caption{Performances de la simulation (tâche C)}
  \begin{tabular}{lll}
    \toprule
    Critère & Valeur obtenue & Cahier des charges \\
    \midrule
    Dépassement & $\leq 5\,\%$ & $\leq 5\,\%$ \checkmark \\
    Écart statique & 0 & 0 \checkmark \\
    Temps de réponse à 5\,\% & $T_r \approx 6 \cdot 10^{-3}$\,s & -- \\
    \bottomrule
  \end{tabular}
\end{table}

Le temps de réponse à 5\,\% vaut environ $T_r \approx 6$\,ms, ce qui est cohérent avec les constantes
de temps du système ($T_1 = 8$\,ms).

% --------------------------------------------------------------------------
\section{Tâche D -- Tension de commande $u_{cm}$}
...
% --------------------------------------------------------------------------
\section{Tâches E-F-G -- Implémentation sur l'installation réelle}

\subsection{Implémentation}

Le schéma Simulink \texttt{SIRAu\_PI\_vit.mdl} a été adapté pour intégrer le régulateur PI dimensionné.
Les paramètres ont été chargés via le script \texttt{Param\_PI\_Vit.m} (voir extrait en
annexe~\ref{ann:code}). Le code a ensuite été compilé sous MATLAB 2016a et chargé sur le PC cible
via xPC-Target.

\subsection{Résultats expérimentaux (sans retard, $T_2 = 1$\,ms)}

La figure~\ref{fig:G_pratique} présente la réponse en vitesse mesurée sur l'installation réelle.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{G_fig_pratique.png}
...
Le dépassement observé expérimentalement est d'environ \textbf{7--8\,\%}, contre 5\,\% visé en
simulation. Plusieurs raisons expliquent cet écart :

\begin{itemize}
  \item \textbf{Frottements du moteur} : non modélisés dans la fonction de transfert linéaire, les
    frottements secs ralentissent le moteur en basse vitesse mais n'amortissent pas efficacement
    le dépassement. Ils introduisent une non-linéarité qui modifie le comportement réel par rapport
    au modèle.
  \item \textbf{Retard non pris en compte} : le système réel présente un retard de traitement
    (calcul numérique, conversion CAN/CNA) qui dégrade la marge de phase et augmente le dépassement.
  \item \textbf{Discrétisation} : le régulateur est implémenté en temps discret ($T_E = 1$\,ms),
    ce qui introduit un demi-pas de retard supplémentaire.
\end{itemize}

Malgré cela, le comportement reste globalement satisfaisant : l'écart statique est nul et la réponse
converge correctement vers la consigne.

% --------------------------------------------------------------------------
\section{Tâche H -- Redimensionnement avec prise en compte du retard}

\subsection{Modélisation du retard}

Un retard de $\tau = 0{,}5$\,ms est introduit dans la boucle (figure~\ref{fig:schema_retard}).
...
Ce retard dégrade la marge de phase et nécessite de recalculer le gain $K_p$.

\subsection{Nouveau dimensionnement}

Avec la prise en compte du retard, la constante de temps effective augmente. On prend désormais
...
\subsection{Simulation avec retard}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{fig_HI_theorique.png}
  \caption{Réponse simulée complète avec retard (vue globale)}
  \label{fig:HI_th2}
\end{figure}

La simulation confirme que le dépassement est ramené à un niveau acceptable ($\leq 5\,\%$) avec le
nouveau jeu de paramètres.

% --------------------------------------------------------------------------
\section{Tâche I -- Résultats expérimentaux avec retard}
...
% --------------------------------------------------------------------------
\appendix
\section{Code MATLAB -- Paramètres du régulateur}
\label{ann:code}

\begin{lstlisting}[caption={Param\_PI\_Vit.m -- Paramètres du régulateur PI}]
%% Données pour labo moteur DC vitesse
%  sur schema SIRAu_PI_vit.mdl

%% Général
TE = 1e-3;          % Temps d'échantillonnage [s]
Temps_manip = 5;    % Durée de la manipulation [s]

%% Capteur de vitesse
Kmes = 10.7e-3;     % Gain tachymètre [V.s/rad]

%% Paramètres du système
Kmot = 4.53e3;      % Gain moteur [rad/(s.Nm)]
Kele = 20.8e-3;     % Gain amplificateur [Nm/V]
T1   = 8e-3;        % Constante de temps moteur [s]

% G (sans retard) :
% T2 = 1e-3;
% H (avec retard 0.5 ms) :
T2 = 0.0015;

Ks = Kmot * Kele * Kmes;  % Gain statique de boucle

%% Dimensionnement du régulateur PI
Tn = 8e-3;          % Compensation du pôle dominant (= T1)
Ti = 2 * Ks * T2;   % Constante intégrale [s]
Kp = Tn / Ti;       % Gain proportionnel
